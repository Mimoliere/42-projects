...
/*   Updated: 2025/04/11 14:08:20 by bguerrou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/so_long.h"

void	borders(char **map)
{
	int	lines;
	int	columns;
	int	max_lines;
	...
	if (max_column <= max_lines || max_lines == 0 || max_column == 0)
		return (free_arr(map), map_error());
	while (++lines < max_lines)
	{
		...
		while (map[lines][++columns] && columns < max_column)
			if (((lines == 0 || lines == max_lines - 1)
					&& map[lines][columns] != '1')
					|| ((lines != 0 && lines != max_lines - 1)
					&& !acceptable_chars(map[lines][columns])))
				return (free_arr(map), map_error());
...
int	recursive_path(char **clone, int row, int column, char **map)
{
	clone[row][column] = 'T';
...
void	finishable(char **clone, char **map)
{
	int	rows;
	int	columns;
	...
	if (count_element(map, 'P') != 1 || count_element(map, 'E') != 1
		...
		return (free_arr(map), free_arr(clone), map_error());
	rows = -1;
	while (clone[++rows])
	{
		columns = -1;
...
void	is_valid_map(char **map, int fd)
{
	int		row;
	int		column;
	char	**clone;
